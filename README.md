...
## Results
We were able to develop two silos: the first is the query tool where we used the Entrez database to identify and analyze genome sequences, and the second is the actual Ziro Studio Visualization. We were able to develop these in parallel, but given the scope and timing of the project, we are still under development to build the AI capabilities to integrate the two silos. The Jupyter Notebook, attached below can access the NCBI Database and is able to spit out the best outputs we can identify, based on our input conditions. We can specify specific diseases, proteins, or gene names, and identify the entire gene/nucleotide sequence, contingent on the gene being present in the database. With the Ziro Studio Visualization, which can be found at https://zirostudio.com/studio?code=kmAzXXuNTvDi0rL, we were able to develop a preliminary model to identify and alter point mutated genomes via a simplistic CRISPR character. We look to leverage this visualization as a baseline to develop more complex models, leveraging Alpha-Fold 2 and ESM-Fold to better understand the protein development from mutated genes, alongside our models. We also look to leverage these results to build more AI/ML algorithms to predict the folding of mRNA models, something that is currently underway. 
